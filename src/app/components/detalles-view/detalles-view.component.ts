...
export class DetallesViewComponent implements OnInit {
  ...
  async generarPDF() {
    ...
    // Definir las posiciones y dimensiones de elementos en la página
    const xPosition = 200;
    const yPosition = 650;
    const xPositio = 70;
    const yPositio = 640;
    ...
    // Cargar y embeber la primera imagen en el documento PDF
    ...
    const data = this.userCollectionData.map((calculo) => [
      calculo.alias,
      calculo.cantidad.toString(), // Convertir a string
      calculo.cuotas.toString(),   // Convertir a string
      calculo.interes.toString(), // Convertir a string
      calculo.monthly.toString(),  // Convertir a string
      calculo.totalInteres.toString(), // Convertir a string
    ]);
